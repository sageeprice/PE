...
import problems.impl.Euler250;
import problems.impl.Euler193;
import problems.impl.Euler216;
...
public class EulerSlowTests {
  ...
  public void testSlowProblems() {
    ...
    // 1.5 seconds.
    assertEquals(new Euler193().solve(), "684465067343069");
    // ~15 seconds.
    assertEquals(new Euler204().solve(), "2944730");
    // ~5 seconds.
    assertEquals(new Euler206().solve(), "1389019170");
    // <20 seconds
    assertEquals(new Euler216().solve(), "5437849");
    // 1.5 seconds
    assertEquals(new Euler231().solve(), "7526965179680");
    // 2 seconds.
    assertEquals(new Euler249().solve(), "9275262564250418");
    // 1.5 seconds.
    assertEquals(new Euler250().solve(), "1425480602091519");
    // ~15 seconds
